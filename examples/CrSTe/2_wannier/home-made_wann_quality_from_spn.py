...
E_F = np.loadtxt('FERMI_ENERGY.in')
...
def compare_eigs_bandstructure_at_exact_kpts(dft_bands, wann_bands, num_kpoints, f_name_out='WannierBerri_quality_error.dat'):
...
error_by_energy = compare_eigs_bandstructure_at_exact_kpts(dft_bands, E_to_compare_with_duplicates, num_kpoints_dft, f_name_out='home-made_quality_error.dat')

plot_err_vs_energy(error_by_energy, Ef=0, title="Wannierization RMS error vs. energy", fig_name_out="wannier_quality_error_by_energy_home-made.png")
...
np.savetxt("home-made_quality_error_S_E.dat", error_E_S_by_energy, header="E (eV)\t|Delta E| (eV)\t|Delta S_x|\t|Delta S_y|\t|Delta S_z|")
...
plt.savefig("ERRORS_all_home-made.png", dpi=400)
plt.close()
...
mean_error_frozen_window = integrate_error(error_E_S_by_energy, E_min=dis_froz_min-E_F, E_max=dis_froz_max-E_F)
...
with open("error_home-made_integrated.dat", 'w') as fw:
    ...
    fw.write(f"in the frozen window ({dis_froz_min-E_F:.3f} to {dis_froz_max-E_F:.3f} eV)\t" + '\t'.join([f"{val:.6f}" for val in mean_error_frozen_window]) + '\n')
...
np.savetxt("home-made_Sabs_vs_E.dat", E_S_abs, header=header)
...
plt.tight_layout()
plt.savefig(plot_title+"_vs_E_home-made.png", dpi=400)
plt.close()
...
plt.tight_layout()
plt.savefig(plot_title+"_S_histogram_home-made.png", dpi=400)
plt.close()
